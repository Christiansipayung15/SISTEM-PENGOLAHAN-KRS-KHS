...
class RekeningTabungan extends RekeningDasar implements Printable {
    ...
    public function getJenisRekening(): string { return 'Tabungan'; }

    public function cetakStruk(): string {
        return "<br>--- STRUK TABUNGAN ---<br>No. Rek : {$this->nomorRekening}<br>Nama : {$this->namaNasabah}<br>Saldo : Rp " . number_format($this->saldo, 0, ',', '.') . "<br>Saldo Min: Rp " . number_format($this->saldoMinimum, 0, ',', '.') . "<br>---------------------<br>";
    }
}